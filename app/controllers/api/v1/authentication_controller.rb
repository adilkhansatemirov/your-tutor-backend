class Api::V1::AuthenticationController < ApplicationController
  ...
  def create
    ...
    if user.save
      ...
      TutorDetail.create(user_id: user.id, sign_up_code: generated_password)
  ...
  def enter_sign_up_code
    ...
    if user.tutor_detail.sign_up_code == enter_code_params[:sign_up_code]
      user.tutor_detail.update_attribute(:sign_up_code_entered, true)
      Tutor::NotificationsService.notify_about_new_tutor(user)
      ...
      if user.role === 'student'
        ...
        return render json: {
          data: {
            ...
            has_payment_info: !student_detail.stripe_customer_id.nil?
          },
          token: user.token
        } 
      elsif user.role === 'tutor'
        if user && !user.tutor_detail.sign_up_code_entered
          render json: {
              success: false,
        ...
        tutor_detail = user.tutor_detail
    ...
    if user.role === 'student'
      return render json: {
        data: {
          user: user,
          student_detail: user.student_detail,
          has_payment_info: !user.student_detail.stripe_customer_id.nil?
        }
      }
    end
